...
module.exports = {
	...
	async execute(interaction) {
        ...
        {
            let list = "";
            for(let i = 0; i < guild.list.length; i++)
            {
                list += `<#${guild.list[i]}>\n`;
            }
            return await interaction.reply({ content: `List type is ${guild.listType == 0? `black` : `white`} list and has ${guild.list.length} channels in it.\n${list}`, ephemeral: true });
        }
